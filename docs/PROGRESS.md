...
- 2026-08-08 unsigned-macos-alpha: the poc's packaging gaps closed for a downloadable build — icon set (instrument countdown mark) wired into tauri.conf (empty bundle.icon previously broke bundling), capabilities/default.json (runtime_call needs no acl entry: no app manifest in build.rs, verified against tauri 2.11.5 source), Cargo.lock committed, lib.rs spawns the sidecar from the ci-assembled Resources/sidecar tree (node + pnpm-deployed runtime + companion, --seed-first-party, sidecar.log) with MORTAL_NODE/MORTAL_RUNTIME_DIR overrides still winning so the dev flow is untouched; the deploy uses the hoisted linker because the tauri resource copier breaks pnpm's symlink farm (caught by the launch smoke: zod unresolvable inside the .app; zero-symlink assertion now gates assembly); release-macos.yml builds the unsigned .dmg on the macos runner (sidecar boot-proof, tauri build, ad-hoc seal, hdiutil, launch smoke: cold ~/.mortal, /v1/health answers, app process stays up) — the shell's first-ever compile caught one E0597 in the day-1 window-destroy handler, fixed; unsigned is a decision (no apple developer enrollment until demonstrated interest) and every download surface prints it; full pnpm check green (258 tests unchanged)
